...
pathData = 'L3_simData_10ms_1'; % name of the folder where simulated data
% is stored
...
%% load LFP data
% supra-CF stimulus
load(fullfile(pathData,'SimData_r1_PS2200_c1.mat'), 'LFP','ze')
LFP_1 = LFP;
% threshold CF stimulus
load(fullfile(pathData,'SimData_r1_PS2200_c2.mat'), 'LFP')
LFP_2 = LFP;
...
Ve_1 = eegfilt(LFP_1, Fs, 0, 100); % [mv] 
...
font = 10;
figure;
LFPmax = 8*max(abs(VeD_1.*1e3),[],'all'); % scale for the LFP plot
...
hold on
plot((599-10+80)-[20 40], [-0.03; -0.03] + 0.5, '-k', 'LineWidth', 1.5);  % 
% time scale
hold on
...
figure;
LFPmax = 10*max(abs(VeD_2.*1e3),[],'all'); % scale for the LFP plot
plot(ts_out.*1e3, (ze.*1e3.*ones(length(ts_out), length(ze)))' + 0.5,'-', ...
    'Color', [0.5 0.5 0.5], 'linewidth', 1)
hold on
for ii = 1:Ne
...
max_CSD = max(abs(iCSD_2.*1e-3), [], 'all');
bar_min = -max_CSD;
bar_max = max_CSD;
caxis([bar_min bar_max]);
c.Label.FontSize = font;
...
d_CSD_1 = (-(zs_1.*1e-3 - (((h*Ne)/2)+a))'.*mean(diff(zs_1.*1e-3)))'*...
    iCSD_1(:,590:(1e3/Fs_new):699)*(pi*(rc^2)).*1e9; % nA*m
% baseline correction
d_CSD_1 = d_CSD_1 - mean(d_CSD_1(1:9));

d_CSD_2 = (-(zs_2.*1e-3 - (((h*Ne)/2)+a))'.*mean(diff(zs_2.*1e-3)))'*...
    iCSD_2(:,590:(1e3/Fs_new):699)*(pi*(rc^2)).*1e9; % nA*m
% baseline correction
...
% load dipolar moments (column coordinate system)
load(fullfile(path2CellsData_1, 'dipoles_rotated_neurons', ...
    'column_eq_dip.mat'),'d');
d_1 = d;
load(fullfile(path2CellsData_2, 'dipoles_rotated_neurons', ...
    'column_eq_dip.mat'),'d');
d_2 = d;

% load dipoles' normal
load(fullfile(pathLeadFieldIN, 'sources_coordinates_V4', ...
...
ylim([min(ds_f(3,:))-0.01 max(ds_f(3,:))+0.01])
ylabel({'z'})
xlabel('Time (ms)')
box off
xlim([0 80])
...
for ii = 0:2199
    ...
    load(fullfile(pathLeadFieldIN,'leadField_V4', ...
